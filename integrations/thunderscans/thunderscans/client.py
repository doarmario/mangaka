"""Bounded HTTP transport and parsers for the public Thunder Scans pages."""
...
IMAGE_HOSTS = {"en-thunderscans.com"}
SLUG = re.compile(r"(?:[a-zA-Z0-9_-]|%[0-9a-fA-F]{2})+\Z")
...
def slug(value):
    if not isinstance(value, str) or not SLUG.fullmatch(value) or len(value) > 800:
    ...
    for _ in range(8):
        ...
        if expanded == decoded:
            return value
        decoded = expanded
    raise SourceError("Codificação de identificador inválida.", 400)


def remote_id(url, kind="manga"):
    parsed = urlsplit(checked_url(urljoin(BASE_URL, url)))
    pattern = r"/comics/([^/]+)/?" if kind == "manga" else r"/([^/]+)/?"
        ...
        try:
            for _ in range(5):
                ...
                with requests.get(url, timeout=(3, 20), allow_redirects=False, stream=True,
                                  headers={"User-Agent": "Mangaka-ThunderScans/0.1",
...
class ThunderScans:
    ...
    def listing(self, page=1, query=None, tag=None):
        def load():
            ...
            # One native page per request. A filtered catalog keeps its native
            # pagination and applies title terms locally instead of N+1 lookups.
            if query and not tag:
                ...
                if tag:
                    params["genre[]"] = tag
            results, next_url = self.listing_page(BASE_URL + path + "?" + urlencode(params))
            if query and tag:
                terms = query.casefold().split()
                results = [r for r in results if all(t in r["title"].casefold() for t in terms)]
            return {"results": results, "total": None, "has_next": next_url is not None}
        return self.cached(("listing", page, query, tag), load)
    ...
    def listing_page(self, url):
        ...
        next_link = doc.select_one(".postbody .pagination a.next, .postbody .hpage a.r")
        next_url = None
        if next_link:
    ...
    def series(self, identifier):
        slug(identifier)
        def load():
            doc = self.document(f"/comics/{identifier}/")
            ...
            for item in chapter_list.select("li"):
                ...
                locked = item.select_one(
                    '.locked, .premium, .fa-lock, [data-bs-target="#lockedChapterModal"], '
                    '[data-target="#lockedChapterModal"]')
                if locked or set(item.get("class", [])) & {"locked", "premium"}:
                    continue
            ...
            return {"id": identifier, "title": title,
                    ...
                    "coverUrl": self.image_url(doc.select_one(".main-info .thumb img")),
    ...
    def pages(self, identifier, chapter_id):
        ...
        def load():
            ...
            parent = doc.select_one('.allc a[href*="/comics/"]')
