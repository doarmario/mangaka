...
from .client import ThunderScans, SourceError, checked_url


def create_app(service=None):
    app = Flask(__name__)
    service = service or ThunderScans()
    app.extensions["thunderscans"] = service

    @app.errorhandler(SourceError)
    def source_error(error):
        response = jsonify(source="thunderscans", error=str(error))
    ...
    @app.before_request
    def source():
        if request.args.get("source", "thunderscans") != "thunderscans":
            raise SourceError("Fonte desconhecida.", 400)

    def payload(data):
        return jsonify(source="thunderscans", **data)
