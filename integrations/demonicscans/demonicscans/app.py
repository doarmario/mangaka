...
from .client import DemonicScans, SourceError, checked_url


def create_app(service=None):
    app = Flask(__name__)
    service = service or DemonicScans()
    app.extensions["demonicscans"] = service

    @app.errorhandler(SourceError)
    def source_error(error):
        response = jsonify(source="demonicscans", error=str(error))
    ...
    @app.before_request
    def source():
        if request.args.get("source", "demonicscans") != "demonicscans":
            raise SourceError("Fonte desconhecida.", 400)

    def payload(data):
        return jsonify(source="demonicscans", **data)
